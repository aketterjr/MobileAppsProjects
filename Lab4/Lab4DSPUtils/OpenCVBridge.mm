...
-(bool)processFinger{
    cv::Mat image_copy;
    char text[50];
    static vector<double>avgBlue;
    static vector<double>avgGreen;
    static vector<double>avgRed;
    Scalar avgPixelIntensity;
    static int counter = 0;
    static double beats = 0.0;
    
    cvtColor(_image, image_copy, CV_BGRA2BGR); // get rid of alpha for processing
    avgPixelIntensity = cv::mean( image_copy );

    if (avgPixelIntensity.val[2] < 45) {
        
        avgBlue.push_back(avgPixelIntensity.val[0]);
        avgGreen.push_back(avgPixelIntensity.val[1]);
        avgRed.push_back(avgPixelIntensity.val[2]);
        // The print statement lets you know when it's capturing data and what it's capturing
        sprintf(text,"Capturing Avg. B: %.0f, G: %.0f, R: %.0f", avgPixelIntensity.val[0],avgPixelIntensity.val[1],avgPixelIntensity.val[2]);
        cv::putText(_image, text, cv::Point(0, 20), FONT_HERSHEY_PLAIN, 2.0, Scalar::all(255), 1, 2);
        ++counter;
        
        if (counter > 450) {
            double max = 0.0;
            for (int i = 0; i < avgRed.size(); ++i) {
            ...
            beats = beats / 450;
            sprintf(text,"BPM: %.0f Avg. B: %.0f, G: %.0f, R: %.0f", beats, avgPixelIntensity.val[0],avgPixelIntensity.val[1],avgPixelIntensity.val[2]);
            cv::putText(_image, text, cv::Point(0, 20), FONT_HERSHEY_PLAIN, 2.0, Scalar::all(255), 1, 2);
        }
